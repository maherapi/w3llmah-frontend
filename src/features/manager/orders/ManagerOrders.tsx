...
export const ManagerOrders: React.FC<Props> = (props) => {
  ...
  const handleOrderDetailsClick = (order: IOrdersItemResponse) => {
    ...
    if(order.request_by_user.userable_type === "Student") {
      history.push("/manager/order/student");
    } else if(order.request_by_user.userable_type === "Teacher") {
      history.push("/manager/order/teacher");
    }
  };
